...
get_platform_port_allocation() {
    ...
    # Calculate platform common ports (offsets 0-19)
    PLATFORM_SSH_PORT=$((BASE_PLATFORM_PORT + 0))
    PLATFORM_MYSQL_PORT=$((BASE_PLATFORM_PORT + 1))
    PLATFORM_POSTGRES_PORT=$((BASE_PLATFORM_PORT + 2))
    PLATFORM_N8N_PORT=$((BASE_PLATFORM_PORT + 6))
    PLATFORM_WORDPRESS1_PORT=$((BASE_PLATFORM_PORT + 11))
    PLATFORM_WORDPRESS2_PORT=$((BASE_PLATFORM_PORT + 12))
    PLATFORM_WORDPRESS3_PORT=$((BASE_PLATFORM_PORT + 13))
    PLATFORM_NEXTJS_BLOG1_PORT=$((BASE_PLATFORM_PORT + 16))
    PLATFORM_NEXTJS_BLOG2_PORT=$((BASE_PLATFORM_PORT + 17))

    # Export for use in template substitution
    export PLATFORM_SN
    export BASE_PLATFORM_PORT
    export PLATFORM_PORT_START="$BASE_PLATFORM_PORT"  # PLATFORM_PORT_START is same as BASE_PLATFORM_PORT
    export PLATFORM_PORT_END
    export PLATFORM_SSH_PORT
    export PLATFORM_MYSQL_PORT
    export PLATFORM_POSTGRES_PORT
    export PLATFORM_N8N_PORT
    export PLATFORM_WORDPRESS1_PORT
    export PLATFORM_WORDPRESS2_PORT
    export PLATFORM_WORDPRESS3_PORT
    export PLATFORM_NEXTJS_BLOG1_PORT
    export PLATFORM_NEXTJS_BLOG2_PORT
}
...
generate_platform_settings() {
    ...
    # Generate secure values (only for JWT and encryption)
    ...
    # Use root .env passwords instead of generating new ones
    ...
    # Export root .env variables for .env file generation
    export DEV_ROOT_PATH="${DEV_ROOT_PATH}"
    ...
    export SSH_ROOT_PASSWORD="${SSH_ROOT_PASSWORD}"
}
...
for file in "${FILES_TO_SUBSTITUTE[@]}"; do
    if [ -f "$file" ]; then
        ...
        sed -i "s/\${PLATFORM_SSH_PORT}/$PLATFORM_SSH_PORT/g" "$file"
        sed -i "s/\${PLATFORM_MYSQL_PORT}/$PLATFORM_MYSQL_PORT/g" "$file"
        sed -i "s/\${PLATFORM_POSTGRES_PORT}/$PLATFORM_POSTGRES_PORT/g" "$file"
        sed -i "s/\${PLATFORM_N8N_PORT}/$PLATFORM_N8N_PORT/g" "$file"
        sed -i "s/\${PLATFORM_WORDPRESS1_PORT}/$PLATFORM_WORDPRESS1_PORT/g" "$file"
        sed -i "s/\${PLATFORM_WORDPRESS2_PORT}/$PLATFORM_WORDPRESS2_PORT/g" "$file"
        sed -i "s/\${PLATFORM_WORDPRESS3_PORT}/$PLATFORM_WORDPRESS3_PORT/g" "$file"
        sed -i "s/\${PLATFORM_NEXTJS_BLOG1_PORT}/$PLATFORM_NEXTJS_BLOG1_PORT/g" "$file"
        sed -i "s/\${PLATFORM_NEXTJS_BLOG2_PORT}/$PLATFORM_NEXTJS_BLOG2_PORT/g" "$file"
...
# Step 2.1: Generate .env file from .env.sample
echo ""
echo "Step 2.1: Generating .env file from .env.sample..."

ENV_SAMPLE_FILE="$PLATFORM_NAME/.env.sample"
ENV_FILE="$PLATFORM_NAME/.env"

if [ -f "$ENV_SAMPLE_FILE" ]; then
    echo "Processing: $ENV_SAMPLE_FILE -> $ENV_FILE"
    ...
    # Substitute root .env variables
    sed -i "s|\${DEV_ROOT_PATH}|$DEV_ROOT_PATH|g" "$ENV_FILE"
    sed -i "s|\${BASE_IP}|$BASE_IP|g" "$ENV_FILE"
    sed -i "s|\${MYSQL_USER}|$MYSQL_USER|g" "$ENV_FILE"
    sed -i "s|\${MYSQL_ROOT_PASSWORD}|$PLATFORM_MYSQL_ROOT_PASSWORD|g" "$ENV_FILE"
    sed -i "s|\${MYSQL_PASSWORD}|$MYSQL_PASSWORD|g" "$ENV_FILE"
    sed -i "s|\${MYSQL_DATABASE}|$MYSQL_DATABASE|g" "$ENV_FILE"
    sed -i "s|\${POSTGRES_USER}|$POSTGRES_USER|g" "$ENV_FILE"
    sed -i "s|\${POSTGRES_PASSWORD}|$PLATFORM_POSTGRES_PASSWORD|g" "$ENV_FILE"
    sed -i "s|\${POSTGRES_DB}|$POSTGRES_DB|g" "$ENV_FILE"
    sed -i "s|\${SSH_ROOT_PASSWORD}|$SSH_ROOT_PASSWORD|g" "$ENV_FILE"

    # Substitute platform port variables
    sed -i "s|\${PLATFORM_PORT_START}|$PLATFORM_PORT_START|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_PORT_END}|$PLATFORM_PORT_END|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_SSH_PORT}|$PLATFORM_SSH_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_MYSQL_PORT}|$PLATFORM_MYSQL_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_POSTGRES_PORT}|$PLATFORM_POSTGRES_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_N8N_PORT}|$PLATFORM_N8N_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_WORDPRESS1_PORT}|$PLATFORM_WORDPRESS1_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_WORDPRESS2_PORT}|$PLATFORM_WORDPRESS2_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_WORDPRESS3_PORT}|$PLATFORM_WORDPRESS3_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_NEXTJS_BLOG1_PORT}|$PLATFORM_NEXTJS_BLOG1_PORT|g" "$ENV_FILE"
    sed -i "s|\${PLATFORM_NEXTJS_BLOG2_PORT}|$PLATFORM_NEXTJS_BLOG2_PORT|g" "$ENV_FILE"

    # Remove backup if substitution was successful
    rm "$ENV_FILE.bak"
    echo "  ✓ Generated .env file with substituted variables"
else
    echo "  ⚠ .env.sample not found: $ENV_SAMPLE_FILE"
fi

echo ".env file generation completed"
